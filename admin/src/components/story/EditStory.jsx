...
function EditStory() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [authorsResponse, categoriesResponse, storyResponse] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL}/author/list`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${import.meta.env.VITE_API_URL}/category/data`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${import.meta.env.VITE_API_URL}/story/detail/${id}`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      ...
      setStory({
        ...
        categories: storyData.categories.map((cat) => ({ value: cat.id, label: cat.name })),
      });

      if (storyData.story_picture) {
        setImage(storyData.story_picture.path); // Set preview image
        setTitle(storyData.story_picture.title || ""); // Set image title
      }
    } catch (error) {
  ...
  const handleAuthorChange = (selectedOption) => {
    setStory({ ...story, author_name: selectedOption.value });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (story.story_picture) {
        ...
        if (responseImage?.data?.status < 400) {
    ...
    <div className="w-full bg-white rounded-xl overflow-auto h-screen flex flex-col p-6 font-mulish">
      ...
      <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-2">
                ...
                {errors.author_name && <span className="text-xl text-error-outline">{errors.author_name}</span>}
            </div>
