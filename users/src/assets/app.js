...
function showFullTabContent() {
  ...
  if (productDetailInfo) {
    ...
    if (productDetailInfoMore) {
      ...
      const collapse = productDetailInfoMore.querySelector(
        ".info-more--collapse"
      );
...
function collapseDescription() {
  const productDetailInfoTabContent = document.querySelector(
...
document.addEventListener("click", function (e) {
  if (
    e.target.classList.contains("info-more--more") ||
    e.target.closest(".info-more--more")
  ) {
    showFullTabContent();
  }

  if (
    e.target.classList.contains("info-more--collapse") ||
    e.target.closest(".info-more--collapse")
  ) {
    collapseDescription();
  }
});
...
$(document).ready(function () {
  ...
  if (selectStoriesHot) {
      ...
      fetch(route("get.list.story.hot"), {
        ...
        .then((data) => {
          if (data.success) {
            var html = $(data.html);
            var list = $(
              ".section-stories-hot__list:not(.wrapper-skeleton)",
              html
            );
            $(".section-stories-hot__list:not(.wrapper-skeleton)").replaceWith(
              list
            );
    ...
    $(document).ready(function () {
      ...
      const savedTheme = getLocalStorage("bg_color");

      // Nếu có giá trị trong localStorage, kiểm tra và áp dụng chế độ
      if (savedTheme === "dark") {
        $("body").addClass("dark-theme");
        $(".theme_mode").prop("checked", true); // Đánh dấu checkbox là checked
      } else {
        $("body").removeClass("dark-theme");
        $(".theme_mode").prop("checked", false); // Đảm bảo checkbox không checked
      }

      // Lắng nghe sự kiện thay đổi của checkbox để thay đổi chế độ
      $(".theme_mode").on("change", function () {
        let valueThemeMode = $(this).is(":checked") ? "dark" : "light";

        // Lưu trạng thái vào localStorage
        setLocalStorage("bg_color", valueThemeMode);

        // Thêm/loại bỏ lớp dark-theme từ body
        if ($(this).is(":checked")) {
          $("body").addClass("dark-theme");
        } else {
          $("body").removeClass("dark-theme");
    ...
    let x = setInterval(() => {
      ...
      if (!selectStoriesHot) {
        ...
        if (valueSelected == null) {
          $(".select-stories-hot option:first")
            .next()
            .attr("selected", "selected");
        } else {
          $(`.select-stories-hot option[value="${valueSelected}"]`)
            .next()
            .attr("selected", "selected");
        }
        ...
        if ($(".select-stories-hot").val() != "Tất cả") {
          ...
          $(".select-stories-hot option:selected")
            .next()
            .attr("selected", "selected");
        }
      }
